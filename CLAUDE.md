...
- **000161** — add `awaiting_customer boolean` to `public_dashboard_projects` mirroring `dashboard_tile_counts.awaiting_customer` so the tile count and the click-through list use the same predicate (PV-2026W19-015). View dropped + recreated.
...
- **000165** — restore owner-update on `profiles` (revoked in 000036). Policy ships with a `WITH CHECK` that asserts the incoming `role` matches the currently-stored value via a self-subquery, so a direct API call trying to flip role from `'designer'` to `'admin'` fails the check at the database layer. Designers can now edit `full_name`, `designer_initials`, and `designer_colour` from the self-service profile modal.
