...
type BadgeState = { count: number; personal: boolean }

let applied: BadgeState = { count: 0, personal: false }
...
function writeTitle(next: string): void {
  ourTitle = next
  document.title = next
}

function decorate(base: string, { count, personal }: BadgeState): string {
...
function ensureTitleObserver(): void {
  ...
  titleObserver = new MutationObserver(() => {
    if (document.title === ourTitle) return // our own write, echoed back
    baseTitle = document.title.replace(TITLE_PREFIX, '')
    if (applied.count > 0) writeTitle(decorate(baseTitle, applied))
  })
...
function loadFavicon(href: string): Promise<HTMLImageElement | null> {
...
async function applyFavicon(state: BadgeState): Promise<void> {
  if (typeof document === 'undefined' || faviconLoadFailed) return
  const link = faviconLink()
  if (!link) return
  if (faviconOriginal === null) faviconOriginal = link.getAttribute('href')
  if (!faviconOriginal) return

  if (state.count <= 0) {
    if (link.getAttribute('href') !== faviconOriginal) setFaviconHref(faviconOriginal)
    return
  }
...
function setFaviconHref(href: string): void {
  ...
  if (href.startsWith('data:image/png')) next.setAttribute('type', 'image/png')
  else if (faviconOriginal === href) next.removeAttribute('type')
  link.replaceWith(next)
}

/* -------------------------------------------------------------- app badge */

function applyAppBadge(state: BadgeState): void {
  const nav = navigator as Navigator & {
    setAppBadge?: (n?: number) => Promise<void>
    clearAppBadge?: () => Promise<void>
  }
  try {
    if (state.count > 0) void nav.setAppBadge?.(state.count)?.catch(() => {})
 ...
 * nothing has changed.
 */
export function setChatBadge(count: number, personal: boolean): void {
  if (typeof document === 'undefined') return
  const next: BadgeState = { count: Math.max(0, count), personal }
  if (next.count === applied.count && next.personal === applied.personal) return
  applied = next
  applyTitle(next)
