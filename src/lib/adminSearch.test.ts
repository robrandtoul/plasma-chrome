...
test('sidebar nav is derived from the registry', () => {
  ...
  // this only alongside such a job (last: Reorder register — reading and
  // intervening in the back book, which no existing page could do).
  assertEqual(navPaths.length, 16)
})
