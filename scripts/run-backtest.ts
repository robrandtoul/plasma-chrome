...
// Claude Opus 4.8 pricing, USD per million tokens (cost estimate only).
...
function parseArgs(): Args {
  ...
  return {
    ...
    limit: limitIx >= 0 ? Number.parseInt(argv[limitIx + 1] ?? '', 10) || null : null,
...
async function main(): Promise<void> {
  ...
  const costUsd = (totalIn / 1e6) * INPUT_PER_MTOK + (totalOut / 1e6) * OUTPUT_PER_MTOK
  writeFileSync(join(reportDir, 'results.json'), JSON.stringify(results, null, 2))
  writeFileSync(join(reportDir, 'report.html'), buildHtmlReport(results, costUsd))
  ...
  console.log(`  tokens: ${totalIn.toLocaleString()} in / ${totalOut.toLocaleString()} out (≈ $${costUsd.toFixed(2)})`)
...
function buildHtmlReport(results: CaseResult[], costUsd: number): string {
  ...
  const cards = results
    .map((r) => {
      ...
      const inputHtml = r.inputThread
        .map((m) => `<div class="msg ${m.role}"><span class="who">${esc(m.author)} (${m.role})</span>${pre(m.body)}</div>`)
        .join('')
      if (!p) {
      ...
      const noteHtml = p.draft?.note_body ? `<h4>Internal note (working)</h4>${pre(p.draft.note_body)}` : ''
      const guardHtml =
...
${noteHtml}${guardHtml}
<p class="meta">summary: ${esc(p.classification.summary)} · tokens ${p.usage.inputTokens}/${p.usage.outputTokens}</p>
</details>`
    })
    .join('\n')
...
<p>${results.length} conversations · estimated API cost $${costUsd.toFixed(2)} ·
