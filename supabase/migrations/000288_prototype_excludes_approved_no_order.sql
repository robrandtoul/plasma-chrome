-- 000288: a prototype order must NOT clear the approved_no_order rule.
--
-- ⚠ TARGET: the MERGED stock-control project (bjvinrzbdrwebylkmbwy), proofs
-- schema. Apply via the dashboard SQL editor / MCP apply_migration, never
-- `supabase db push`. AUTHORED, NOT YET APPLIED — Rob applies this.
--
-- The prototyping service is "a sample BEFORE the big order" — so a customer
-- can pay for a prototype and still need to place the real production order.
-- The approved_no_order needs-attention rule flags an approved proof with no
-- order link sent; if a paid prototype cleared it, that genuine "send the
-- production link" to-do would vanish.
--
-- Fix: the ano CTE's "no order exists" guard gains `and o.order_kind =
-- 'production'`, so only a real production order clears the rule — a prototype
-- order leaves it firing. (The rule ships disabled; this keeps it correct for
-- whenever Rob enables it. The LIVE surface — the Orders page "Links to send"
-- worklist + nav badge — gets the matching production-only filter in
-- src/lib/approvedNoOrder.ts.)
--
-- Body is the verbatim live definition of proofs.proofs_needing_attention()
-- (read via pg_get_functiondef at authoring time) with that single line added —
-- nothing else changed. create-or-replace preserves the existing grants and the
-- SET search_path, so no grant re-statement is needed. order_kind must exist
-- first (000287).
--
-- Trade-off (documented): for a customer who only ever wants 1–3 copies (the
-- prototype IS their whole order), this leaves the proof in "Links to send"
-- until manually handled — acceptable noise versus silently dropping the
-- production follow-up for the dominant "prototype then production" case.
