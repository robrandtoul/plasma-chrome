-- public.create_order_handoff — the direct order hand-off from proof-viewer
-- into Stock Control (docs/order-handoff-spec.md §3.1–§3.4).
--
-- Called by the place-order edge function (service_role). Two modes:
--
--   * p_validate_only = true  → runs every resolution + validation and returns
--     problems/warnings WITHOUT writing anything. This backs the review page's
--     pre-send checks (shadow + live) and never inspects the proofs order's
--     status (shadow calls it after the order has already flipped to fulfilled).
--   * p_validate_only = false → in ONE transaction: inserts the Stock Control
--     job (public.orders + order_lines via create_inhouse_order_from_note, or
--     public.outsourced_orders directly) AND flips proofs.orders to fulfilled
--     with the handoff stamps. An exception rolls the whole thing back — the
--     caller can simply retry. handoff_at is the idempotency key: a re-call for
--     an order that already has it returns already_imported without writing.
--
-- Resolution happens HERE (single SQL implementation, surfaced to the designer
-- through the validate-only response): explicit proofs.materials.stock_material_id
-- mapping first, then an exact/loose name match — never token-fuzzy; anything
-- ambiguous is a named problem, not a guess. Letterpress passes its colour trio
-- through to create_inhouse_order_from_note → _resolve_letterpress_combo.
--
-- The composed job-card notes strings mirror the shapes the webhook parsers
-- write today (buildSpecNotes' " | " join in-house; "thickness · finish"
-- outsourced) so job cards look identical either way — PLUS the prototype
-- marker and the designer's free note, which the parsers used to drop.
--
-- ⚠ Public-schema functions default to EXECUTE for everyone — the revoke at the
-- bottom is mandatory (inverse of the proofs-schema no-default-grants footgun).
--
-- ⚠ The outsourced ship/arrival defaulting replicates
-- create_outsourced_order_from_email (000331) — keep the two in lockstep.
--
-- Target: the merged stock-control project. Apply via MCP / dashboard SQL
-- editor per the house workflow — never CLI push. Apply AFTER 000331 + 000332.
...
begin
      ...
      if v_map_target is not null then
        ...
        if v_match_count = 0 then
          select count(*), min(m.id::text)::uuid, min(m.name)
            ...
            from public.materials m
           where not m.archived and not m.is_custom
             and regexp_replace(lower(regexp_replace(m.name, '\s+', ' ', 'g')), '\s+(plastic|card|cards)$', '')
               = regexp_replace(v_norm_target, '\s+(plastic|card|cards)$', '');
        end if;
        if v_match_count = 0 then
  ...
  if v_handoff_at is not null then
    return jsonb_build_object(
      ...
      'resolution', jsonb_build_object('sc_material_id', v_sc_material_id, 'sc_material_name', v_sc_material_name, 'product_type_id', v_product_type_id)
    );
  end if;
