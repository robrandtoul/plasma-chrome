-- 000334: wood/size-aware loose match for the direct hand-off resolver.
--
-- Full re-emit of public.create_order_handoff (000333) with ONE change: the
-- loose name-match pass now also strips a trailing size token ("3mm") from
-- BOTH sides, so an in-house wood order's card line ("Black Walnut") resolves
-- to Stock Control's per-size row ("Black Walnut 3mm") — the old fuzzy parser
-- tolerated this; the exact matcher didn't (found on live 2026-07-20: every SC
-- wood/acrylic row carries a trailing size). If SC ever stocks two sizes of
-- one material the strip makes the match ambiguous and resolution fails to a
-- named material_ambiguous problem demanding an explicit mapping — the right
-- failure mode. Everything else is byte-identical to 000333.
--
-- Target: the merged stock-control project. Apply via MCP / dashboard SQL
-- editor per the house workflow — never CLI push. Apply AFTER 000333.
