-- 000360: put the artwork link (and gilding) on the Stock Control job card.
--
-- Full re-emit of public.create_order_handoff (000345) with two additions to the
-- composed job-card `notes`, both closing real information gaps found while
-- planning Phase 3 (freeing the message wording):
--
--  1. ARTWORK LINK. Verified on live 2026-07-27: of 126 in-house jobs created in
--     the last 90 days, 85 carry artwork — and ZERO job cards carry an artwork
--     link in `notes`. Files reach a job card today only because Stock Control's
--     webhook parser mirrors the note's ATTACHMENTS, which requires the note to
--     still parse. That makes artwork the hard blocker on freeing the in-house
--     wording. Emitting `Artwork: <dropbox url>` onto the job card converts the
--     worst case from "no artwork at all" into "click the link", and is worth
--     having immediately regardless of Phase 3. The url is already in the
--     contract payload (`artwork.dropbox_url`) and was simply being ignored.
--
--  2. GILDING. `_resolve_letterpress_combo` takes only the three colours, so a
--     GILDED order and a plain one with the same trio resolve to the identical
--     material — the sole signal to the workshop is the " + gilding" text on the
--     note's Card line. That makes gilding invisible on the direct path the
--     moment the wording is free (and spec §6 explicitly proposes retiring that
--     phrasing). Carry it structurally instead.
--
-- Additive only: existing job cards are unaffected, and nothing reads `notes`
-- programmatically. Applies to both routes so an outsourced job card gains the
-- link too.
--
-- Target: the merged stock-control project. Apply via MCP / dashboard SQL editor
-- per the house workflow — never CLI push. Apply AFTER 000345.
