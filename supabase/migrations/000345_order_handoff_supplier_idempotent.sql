-- 000345: make the SUPPLIER insert idempotent (Phase 2 hardening).
--
-- Full re-emit of public.create_order_handoff (000335) with ONE change: the
-- supplier branch now dedupes before inserting, exactly as the in-house branch
-- has always done via create_inhouse_order_from_note's inhouse_order_no lookup.
--
-- WHY: `handoff_at` on proofs.orders was the direct path's ONLY dedupe key, and
-- the supplier branch did a bare INSERT. Two ways that bit (both found in the
-- 2026-07-24 Phase 2 review, before go-live):
--   1. Two designers confirming the same order concurrently — the RPC serialises
--      on `select ... for update`, so the loser gets already_imported and writes
--      nothing, but only the in-house branch ever SET already_imported, so a
--      caller couldn't tell.
--   2. A revision re-place clears handoff_at (a re-place must create a NEW job);
--      if the RPC then commits but the HTTP call times out, the retry clears the
--      freshly-written stamp again and inserts a SECOND supplier job.
-- A duplicate supplier job means a duplicate production run and a real bill, so
-- the insert must be idempotent in its own right rather than relying on a stamp
-- written by the caller.
--
-- THE KEY: trimmed customer_order_ref + supplier_id + quantity, restricted to
-- non-cancelled rows this direct path created (import_source = 'direct') in the
-- last 14 days — the same signature migration 000331's webhook adoption uses.
-- Deliberately NOT a unique index: one order number legitimately carries several
-- live supplier jobs (verified on live — multi-material orders differ by
-- supplier or quantity). A genuine re-place is unaffected because the human
-- cancels the old job first, and cancelled rows are excluded.
--
-- Also sets v_already on the supplier path so `already_imported` is meaningful
-- on BOTH routes; the caller relies on it to decide whether to send.
--
-- Target: the merged stock-control project. Apply via MCP / dashboard SQL editor
-- per the house workflow — never CLI push. Apply AFTER 000335.
...
begin
  ...
  if v_route = 'in_house' then
    ...
    -- Idempotency for the supplier route (see the 000345 header). A live job
    -- this direct path already created for the same ref + supplier + quantity is
    -- THIS order's job: return it instead of raising a duplicate production run.
    select id into v_pre_existing
      from outsourced_orders
