-- 000383: teach the direct hand-off about combined supplier batches
-- ("base cards supplied under another order's batch", pairs with 000382).
--
-- A blanks-source order hands off IN-HOUSE (the finishing job — foiling on
-- supplier-made blanks) but its material is a SUPPLIER material, which Stock
-- Control's in-house materials list has never needed to carry: public.materials
-- is the sheet-stock catalogue (plastics, acrylics, woods, letterpress combos)
-- and holds nothing for e.g. Standard Paper. Verified on live 2026-08-04:
-- paper_standard has stock_material_id NULL and no name match, so an in-house
-- hand-off for it would die with `material_unmapped`. Meanwhile the workshop's
-- own hand-keyed foiling jobs on supplier blanks routinely have NO material
-- lines at all (403917 / 403901 / 403882, all completed) — supplier blanks
-- aren't sheet stock to allocate. So for a blanks-source hand-off, and ONLY
-- then, an unresolvable material downgrades from a problem to a warning and
-- the job is created without a material line, matching existing practice.
-- Normal in-house orders keep the strict rule unchanged.
--
-- Two changes:
--   1. public.create_inhouse_order_from_note gains `p_allow_no_lines`
--      (default false — every existing caller, including the webhook parser,
--      is byte-identical in behaviour). Adding a parameter would CREATE AN
--      OVERLOAD under create-or-replace and make existing named-parameter
--      calls ambiguous, so the old signature is DROPPED and the new one
--      created in the same transaction, with the ACL restated (live ACL read
--      2026-08-04: EXECUTE for service_role only).
--   2. public.create_order_handoff re-emitted from the LIVE pg_get_functiondef
--      (read 2026-08-04, md5 8170e1690d6afa99cc0f117ae581fc38 — the live body
--      has drifted from 000345: it carries the "Gilded edges" and job-card
--      "Artwork:" parts a later re-emit added, so 000345 is NOT a valid base).
--      Changes: parse the payload's optional `blanks_source` object (presence
--      = blanks mode; written by place-order via buildHandoffPayload); in
--      blanks mode the four material-resolution failures emit a
--      `material_line_skipped` WARNING instead of a problem; the in-house
--      insert passes an empty lines array + p_allow_no_lines when no material
--      resolved. Everything else byte-identical to live.
--
-- Target: the merged stock-control project. Apply via MCP apply_migration /
-- dashboard SQL editor per the house workflow — never CLI push. Apply AFTER
-- 000382 and BEFORE deploying the updated place-order function. Post-apply
-- smoke test: p_validate_only=true calls for (a) a normal in-house payload,
-- (b) a supplier payload, (c) a blanks_source payload — (a)/(b) must behave
-- exactly as before, (c) must return ok with the material_line_skipped warning.

-- ── 1. create_inhouse_order_from_note: allow a lineless job (blanks mode) ────

drop function if exists public.create_inhouse_order_from_note(text, text, jsonb, bigint, text, text, text);
...
-- The drop discarded the old signature's ACL; restate it for the new one
-- (live ACL 2026-08-04: postgres owner + service_role EXECUTE only).
revoke all on function public.create_inhouse_order_from_note(text, text, jsonb, bigint, text, text, text, boolean) from public, anon, authenticated;
grant execute on function public.create_inhouse_order_from_note(text, text, jsonb, bigint, text, text, text, boolean) to service_role;

-- ── 2. create_order_handoff: blanks_source awareness ─────────────────────────
-- Base: LIVE pg_get_functiondef read 2026-08-04 (NOT the 000345 file — see
-- header). Blanks changes marked with "000383".
