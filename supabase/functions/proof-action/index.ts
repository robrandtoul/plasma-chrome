  ...
  try {
    const res = await fetch(`${baseUrl}/rest/v1/rpc/${fnName}`, {
      ...
      headers: {
        ...
        // project; under one PostgREST the RPC names would otherwise
        // resolve against the default public schema, so the schema must
        // be selected explicitly. RPC POSTs read via Accept-Profile and
        // route the request body via Content-Profile.
        'Accept-Profile': 'qr',
        'Content-Profile': 'qr',
      },
    ...
    if (token == null || primaryCustomerId == null) {
      ...
      if (senderId == null) {
        ...
        if (templateErr || !templateRow) {
          ...
          //
          // Hidden INLINE (not via a background task): Supabase can tear
          // the worker down the moment the response is returned, which
          // killed the earlier EdgeRuntime.waitUntil + delay approach
          // before it ever ran. Running the PATCH here, inside the
          // request, is deterministic. It's safe to fire immediately
          // after the reply: Help Scout commits the outbound email at
          // reply-creation time, so hiding the thread afterwards never
          // unsends it (confirmed live — hidden confirmation replies
          // still carry an `openedAt`, i.e. the customer received and
          // opened them). Own try/catch so a hide miss is logged as
          // exactly that and never fails the customer's request — the
          // reply and its email are already durable here. Skipped when
          // the thread id couldn't be parsed (0).
          if (replyThreadId > 0) {
            try {
