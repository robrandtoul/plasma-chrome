...
// Sign-off phrases that begin a signature block. Everything from the first
// one (at the start of a line) to the end is treated as signature and dropped.
const SIGNOFF_RE =
  /^[\s>]*(kind(est)? regards|warm(est)? regards|best regards|best wishes|many thanks|thanks( (so|very) much)?|thank you|with thanks|cheers|regards|yours( sincerely| faithfully)?|speak soon|all the best|best,)\b/im
