...
// Comfortably above the rendered sizes for retina headroom: THUMB_WIDTH for
// the small row thumbnail (~72px box), PREVIEW_WIDTH for the hover popover.
// Width-only transforms preserve aspect ratio (CSS object-contain handles the
// final fit in the box).
...
Deno.serve(async (req) => {
  ...
  // Sign one URL, with a transform; on any transform failure fall back to a
  // plain signed URL so a rendition the transformer can't handle (e.g. an SVG)
  // still shows — just at full size — rather than vanishing.
