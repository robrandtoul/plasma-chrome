...
export function HeaderBar(props) {
    ...
    return (_jsxs("div", { className: "pd-chrome__bar", children: [appsVisible || apps.length < 2 ? (_jsxs("span", { className: "pd-chrome__app--static", children: [_jsx("span", { className: "pd-chrome__app-mark", children: appGlyph(currentApp, 15, appName) }), _jsx("span", { className: "pd-chrome__app-name", children: appName })] })) : (_jsx(AppMenu, { apps: apps, currentApp: currentApp, appName: appName, open: openMenu === 'apps', onOpen: () => setOpenMenu('apps'), onClose: () => setOpenMenu((current) => (current === 'apps' ? null : current)), appsVisible: appsVisible, onAppsVisibleChange: onAppsVisibleChange })), _jsx("span", { className: "pd-chrome__divider" }), _jsx("ul", { className: "pd-chrome__nav", role: "list", "aria-label": appName + ' navigation', children: nav.map((item) => {
                ...
                : null, _jsx("span", { className: "pd-chrome__divider pd-chrome__divider--account" }), _jsx(AccountMenu, { showAppsPreference: apps.length >= 2, user: user, open: openMenu === 'account', onOpen: () => setOpenMenu('account'), onClose: () => setOpenMenu((current) => (current === 'account' ? null : current)), appsVisible: appsVisible, onAppsVisibleChange: onAppsVisibleChange, onSignOut: onSignOut, onEditProfile: onEditProfile, accountLinks: accountLinks, accountActions: accountActions, linkComponent: linkComponent })] }));
}
