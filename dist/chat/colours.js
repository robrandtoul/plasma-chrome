...
export function isDesignerColour(value) {
    return value != null && value in COLOUR_CSS;
}
/** Solid identity colour — avatar fills, badge backgrounds, chip text. */
export function designerColourCss(colour, fallback) {
    if (isDesignerColour(colour))
        return COLOUR_CSS[colour];
    return fallback ? COLOUR_CSS[fallback] : UNKNOWN;
}
export function designerColourLabel(colour) {
 ...
 */
export function designerTint(colour, percent = 14) {
