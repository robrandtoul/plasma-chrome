...
const STATUS_RANK = { online: 0, idle: 1, away: 2, busy: 3 };
/* ── Preferences ────────────────────────────────────────────────────────
...
function keyFor(prefix, name) {
    return `${prefix}-${name}`;
}
function readLocal(prefix, name) {
    try {
...
function writeLocal(prefix, name, value) {
    try {
        if (value === null)
            localStorage.removeItem(keyFor(prefix, name));
        else
            localStorage.setItem(keyFor(prefix, name), value);
    }
    catch {
...
const TeamChatContext = createContext(DEFAULT);
...
export function TeamChatProvider({ config, children, }) {
    ...
    const cfg = useMemo(() => resolveChatConfig(config), 
    ...
    const [activeThread, setActiveThreadState] = useState(() => readThread(prefix));
    ...
    const dmReadsRef = useRef({});
    const activeThreadRef = useRef(readThread(prefix));
    ...
    const plainWindowRef = useRef(null);
    ...
    function computeStatus() {
        if (manualRef.current === 'away')
            return 'away';
        if (manualRef.current === 'busy')
            return 'busy';
    ...
    function refreshStatus() {
        ...
        if (next === myStatusRef.current)
            return;
        myStatusRef.current = next;
        setMyStatus(next);
        const ch = channelRef.current;
        if (ch)
            void ch.track(presencePayload());
    }
    /**
     ...
     */
    function persistPref(patch, local) {
        const prefix = cfgRef.current.storagePrefix;
        for (const [name, val] of local)
            writeLocal(prefix, name, val);
        ...
        void dbRef.current
            ...
            .then(({ error }) => {
            if (error)
                console.warn('[chat] preference did not sync across apps:', error.message);
        });
    }
    ...
    function stampSeen(thread) {
        ...
        if (thread === 'team') {
            ...
            void dbRef.current
                ...
                .then(({ error }) => {
                if (error)
                    console.error('[chat] team seen stamp failed:', error.message);
            });
            setMentionUnread(0);
        }
        else {
            ...
            void dbRef.current
                ...
                .then(({ error }) => {
                if (error)
                    console.error('[chat] DM read stamp failed:', error.message);
            });
        }
    ...
    // badge seconds later.
    function applyRemoteSeen(thread, at) {
        if (thread === 'team') {
    ...
    function recomputeTyping() {
        ...
        for (const [uid, v] of typingMapRef.current) {
            ...
            if (v.thread === activeThreadRef.current)
                arr.push({ userId: uid, name: v.name });
        }
        setTypingUsers(arr);
    }
    ...
    async function syncMessages() {
        ...
        try {
            ...
            if (fetched.length === 0 && messagesRef.current.length > 0)
                return;
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            if (cancelled)
                return;
            dmReadsRef.current = Object.fromEntries((dmReads ?? []).map((r) => [
                r.peer_id,
                r.seen_at,
            ]));
            ...
            const prefs = rawPrefs && typeof rawPrefs === 'object' && !Array.isArray(rawPrefs)
                ? rawPrefs
                : {};
            ...
            if (restoredThread !== 'team' &&
                !(mem ?? []).some((m) => m.id === restoredThread)) {
                activeThreadRef.current = 'team';
                setActiveThreadState('team');
                persistPref({ thread: 'team' }, [['thread', null]]);
            }
            seenAtRef.current = prof?.team_chat_seen_at ?? null;
            profileRef.current = {
            ...
            const channel = cfgRef.current.client
                ...
                .on('postgres_changes', { event: 'INSERT', schema: cfgRef.current.schema, table: 'team_messages' }, (payload) => {
                ...
                .on('postgres_changes', { event: 'DELETE', schema: cfgRef.current.schema, table: 'team_messages' }, (payload) => {
                const old = payload.old;
                if (old?.id)
                    setMessages((prev) => prev.filter((m) => m.id !== old.id));
            })
                .on('postgres_changes', { event: 'INSERT', schema: cfgRef.current.schema, table: 'team_message_reactions' }, (payload) => {
                const row = payload.new;
                setReactions((prev) => (prev.some((r) => r.id === row.id) ? prev : [...prev, row]));
            })
                .on('postgres_changes', { event: 'DELETE', schema: cfgRef.current.schema, table: 'team_message_reactions' }, (payload) => {
                const old = payload.old;
                if (old?.id)
                ...
                if (!p.to)
                    thread = 'team';
                else if (p.to === userIdRef.current)
                    thread = p.userId;
                if (!thread)
                    return;
        ...
        channel.onmessage = (e) => {
            ...
            switch (msg.kind) {
                ...
                case 'popout-alive':
                    ...
                    if (placementRef.current !== 'popout')
        ...
        if (amPopout) {
            ...
            watchdog = window.setInterval(() => {
                ...
                if (pipWindowRef.current)
                    return; /* that route resets on its own pagehide */
                ...
                if (handle && !handle.closed)
                    return;
                if (popoutIsAlive(remoteBeatRef.current, Date.now()))
                    return;
                remoteBeatRef.current = 0;
                plainWindowRef.current = null;
                setPlacementState(readPlacement(cfgRef.current.storagePrefix));
    ...
    useEffect(() => {
        ...
        function bump() {
            const now = Date.now();
            if (now - lastBump < ACTIVITY_THROTTLE_MS)
                return;
            lastBump = now;
    ...
    async function openPopout() {
        ...
        const win = window.open(popoutPath(cfgRef.current.fullPagePath), cfgRef.current.popoutWindowName, windowFeatures(size, { x: window.screenX, y: window.screenY, width: window.outerWidth }));
    ...
    const value = {
        ...
        loadEarlier: async (thread) => {
            ...
            if (status === 'loading' || status === 'exhausted')
                return 0;
            ...
            if (thread === 'team')
                ...
                q = q.or(`and(author_id.eq.${thread},recipient_id.eq.${uid}),and(author_id.eq.${uid},recipient_id.eq.${thread})`);
            if (anchor)
                q = q.lt('created_at', anchor);
            ...
            if (fresh.length > 0) {
                setMessages((prev) => {
                    ...
                    if (add.length === 0)
                        return prev;
                    return [...add, ...prev].sort((a, b) => a.created_at < b.created_at ? -1 : a.created_at > b.created_at ? 1 : 0);
                });
        ...
        toggleReaction: (messageId, emoji) => {
            ...
            if (!uid)
                return;
            const existing = reactionsRef.current.find((r) => r.message_id === messageId && r.user_id === uid && r.emoji === emoji);
            if (existing) {
                ...
                void dbRef.current
                    ...
                    if (error)
                        setReactions((prev) => prev.some((r) => r.id === existing.id) ? prev : [...prev, existing]);
                });
            }
            else {
                void (async () => {
        ...
        myStatus,
        send: async (body, mentionedUserIds = [], attachments = []) => {
            const uid = userIdRef.current;
            const text = body.trim();
            if ((!text && attachments.length === 0) || !uid)
                return { ok: false };
    ...
    return _jsx(TeamChatContext.Provider, { value: value, children: children });
}
