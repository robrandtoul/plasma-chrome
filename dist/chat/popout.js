 ...
 * a 404.
 */
export function popoutPath(fullPagePath) {
 ...
 */
export function popoutWindowName(prefix) {
...
export function popoutSessionKey(prefix) {
    return `${prefix}-is-popout`;
}
export function popoutSizeKey(prefix) {
 ...
 */
export function syncChannelName(prefix) {
    return `${prefix}-sync`;
}
...
export function clampPopoutSize(size) {
    return {
...
export function isPopoutSearch(search) {
    try {
 ...
 */
export function isPopoutWindow(prefix) {
...
export function popoutIsAlive(lastBeatAt, now) {
    if (!lastBeatAt)
        return false;
 ...
 */
export function readStoredPopoutSize(prefix) {
    try {
        ...
        if (typeof p.w === 'number' && typeof p.h === 'number')
            return clampPopoutSize({ w: p.w, h: p.h });
    }
    catch {
        /* ignore */
    }
...
export function readPopoutSize(prefix) {
    try {
        const raw = localStorage.getItem(popoutSizeKey(prefix));
        if (raw) {
            const p = JSON.parse(raw);
...
export function writePopoutSize(prefix, size) {
 ...
 */
export function windowFeatures(size, anchor = { x: 0, y: 0, width: 0 }) {
 ...
 */
export function preparePopoutDocument(win) {
