...
- the **pay page in group mode** — `OrderPayPage` loads the group + its member orders and renders one line per card with a single combined total and one payment.

**Still excluded (Slice 3):** the unified *customer proof overview* (one link to review/approve several cards together) and the designer set-authoring entry points (§5). Those are about proofing together; all payment-combining is Slice 2.
...
*Prepared 8 Jul 2026 from the Novion design conversation (Slice 2 scope note added 8 Jul). Decisions in §7 are settled; §8 are open. Naming is a placeholder throughout.*
